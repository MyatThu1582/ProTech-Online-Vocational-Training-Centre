...
<body>
  ...
  ?>
  <div class="bannar">
    <h2 class="text-dark" style="font-size:30px; padding-top:21.5%; padding-left:10%; font-weight:bold;">
      <a href="course.php" class="text-dark">Courses</a> / <?php echo $course['name']; ?>
    </h2>
  </div>
        ...
        <div class="col-8" style="">
            ...
            <div class="mt-5 me-5">
              ...
              <?php if (!empty($_SESSION['user_id']) &&!empty($_SESSION['name'])) {
                ...
                $permissioncheckstmt = $pdo->prepare("SELECT * FROM permission WHERE user_id='$user_id'");
                $permissioncheckstmt->execute();
                $permissioncheckdatas = $permissioncheckstmt->fetchall();
                if (!empty($permissioncheckdatas)) {
                  foreach ($permissioncheckdatas as $permissioncheckdata) {
                    if ($permissioncheckdata['course_id'] == $_GET['course_id']) {
                  ?>
